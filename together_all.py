...
for metric_name in metric_names: 
    if "simpson " in metric_name or "trapz " in metric_name:
            ...
            for latit in lat_names:
                for longit in long_names:
                    if " no time t1 no time sample" in metric_name:   
                        distance_predicted[subdir_name][some_file][metric_name][longit + "-" + latit] = compare_traj_and_sample(long_dict[longer_file_name][longit], lat_dict[longer_file_name][latit], range(len(times_cumulative[longer_file_name])), {"long": longitudes, "lat": latitudes, "time": times}, metric_name.replace(" no time t1 no time sample", ""), True, True)
                        continue
                    if " no time t1" in metric_name:    
                        distance_predicted[subdir_name][some_file][metric_name][longit + "-" + latit] = compare_traj_and_sample(long_dict[longer_file_name][longit], lat_dict[longer_file_name][latit], range(len(times_cumulative[longer_file_name])), {"long": longitudes, "lat": latitudes, "time": times}, metric_name.replace(" no time t1", ""), True, False)
                        continue
                    if " no time sample" in metric_name:  
                        distance_predicted[subdir_name][some_file][metric_name][longit + "-" + latit] = compare_traj_and_sample(long_dict[longer_file_name][longit], lat_dict[longer_file_name][latit], range(len(times_cumulative[longer_file_name])), {"long": longitudes, "lat": latitudes, "time": times}, metric_name.replace(" no time sample", ""), False, True)
                        continue
                    if " no scale" in metric_name:    
                        distance_predicted[subdir_name][some_file][metric_name][longit + "-" + latit] = compare_traj_and_sample(long_dict[longer_file_name][longit], lat_dict[longer_file_name][latit], range(len(times_cumulative[longer_file_name])), {"long": longitudes, "lat": latitudes, "time": times}, metric_name.replace(" no scale", ""), False, False, size, False, False, dotsx_original, dotsy_original)
                        continue
                    if " scale" in metric_name:
                        distance_predicted[subdir_name][some_file][metric_name][longit + "-" + latit] = compare_traj_and_sample(long_dict[longer_file_name][longit], lat_dict[longer_file_name][latit], range(len(times_cumulative[longer_file_name])), {"long": longitudes, "lat": latitudes, "time": times}, metric_name.replace(" scale", ""), False, False, size, True, False, dotsx_original, dotsy_original)
                        continue
                    if " offset" in metric_name:    
                        distance_predicted[subdir_name][some_file][metric_name][longit + "-" + latit] = compare_traj_and_sample(long_dict[longer_file_name][longit], lat_dict[longer_file_name][latit], range(len(times_cumulative[longer_file_name])), {"long": longitudes, "lat": latitudes, "time": times}, metric_name.replace(" offset", ""), False, False, size, True, True, dotsx_original, dotsy_original)
                        continue 
