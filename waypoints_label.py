...
def plot_anomaly(long, lat, arr, val_arr, probs, title):
    ...
    for inde in range(len(arr)):
        if inde < len(long) and inde < len(lat) and arr[inde]: 
            min_new = max(0, inde - 10)
            max_new = min(len(long), inde + 10) 
            new_title = title + "=" 
            if inde > 1:
            ...
            plt.plot(long[min_new:max_new], lat[min_new:max_new], color = "blue")
            plt.scatter(long[inde], lat[inde], color = "blue")
            plt.show()
    '''
