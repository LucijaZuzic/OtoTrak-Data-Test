...
def save_plot_longitude_latitudes_for_ride(longitudes, latitudes, image_name):  
    ...
    ax.yaxis.set_tick_params(labeSEft = False)  
    ax.set_xticks([])
    ax.set_yticks([])
    ax.axis('off')
...
def make_SpectralEmbedding(data_matrix):
    df_data_matrix = pd.DataFrame(data = data_matrix)
    SE_data_matrix = manifold.SpectralEmbedding(n_neighbors = 6, n_components = 2)
    SE_data_matrix.fit(df_data_matrix)
    return SE_data_matrix

def make_manifold(SE_data_matrix, data_matrix):
    manifold_2D_SEa = SE_data_matrix.fit_transform(data_matrix)
    manifold_2D_SE = pd.DataFrame(manifold_2D_SEa, columns = ['Component 1', 'Component 2'])
    return manifold_2D_SE
...
def plot_multi_SpectralEmbedding(SE_data_matrix, multi_data_matrix, save_name, title_SpectralEmbedding, label_names):  
    random_color_list = random_colors(len(multi_data_matrix))
    plt.title(title_SpectralEmbedding)
    for data_matrix_index in range(len(multi_data_matrix)):
        ...
        if not os.path.isfile(name_multi_manifold) or use_multi_SpectralEmbedding:
            manifold_2D_SEa = SE_data_matrix.fit_transform(multi_data_matrix[data_matrix_index]) 
            manifold_2D_SE = pd.DataFrame(manifold_2D_SEa, columns = ['Component 1', 'Component 2'])
            save_object(name_multi_manifold, manifold_2D_SE)
        else: 
            manifold_2D_SE = load_object(name_multi_manifold)
        plt.scatter(manifold_2D_SE['Component 1'], manifold_2D_SE['Component 2'], marker = '.', color = random_color_list[data_matrix_index], label = label_names[data_matrix_index])
         
    plt.legend()
    plt.savefig(save_name)
    plt.close()
    
def plot_SpectralEmbedding(manifold_2D_SE, longs_matrix, lats_matrix, save_name):  
    maxx = max(manifold_2D_SE['Component 1'])
    maxy = max(manifold_2D_SE['Component 2'])
    minx = min(manifold_2D_SE['Component 1'])
    miny = min(manifold_2D_SE['Component 2'])
    ...
    for x in range(len(manifold_2D_SE['Component 1'])):
        x_dist_right = maxx - manifold_2D_SE['Component 1'][x]
        x_dist_left = manifold_2D_SE['Component 1'][x] - minx
        x_dist_center = abs(manifold_2D_SE['Component 1'][x] - (maxx + minx) / 2)
        y_dist_top = maxy - manifold_2D_SE['Component 2'][x]
        y_dist_mid = abs(manifold_2D_SE['Component 2'][x] - (maxy + miny) / 2)
        y_dist_bottom = manifold_2D_SE['Component 2'][x] - miny
    ...
    print(manifold_2D_SE.head())

    # Show 2D components plot
    plt.figure(figsize = (20, 20))
    plt.subplot(3, 4, 1)
    plt.title(save_name.split("/")[0])
    plt.scatter(manifold_2D_SE['Component 1'], manifold_2D_SE['Component 2'], marker = '.', color = 'cyan')
    plt.scatter(manifold_2D_SE['Component 1'][top_left_index], manifold_2D_SE['Component 2'][top_left_index], color = 'blue', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_SE['Component 1'][top_center_index], manifold_2D_SE['Component 2'][top_center_index], color = 'orange', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_SE['Component 1'][top_right_index], manifold_2D_SE['Component 2'][top_right_index], color = 'green', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_SE['Component 1'][mid_left_index], manifold_2D_SE['Component 2'][mid_left_index], color = 'red', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_SE['Component 1'][mid_center_index], manifold_2D_SE['Component 2'][mid_center_index], color = 'purple', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_SE['Component 1'][mid_right_index], manifold_2D_SE['Component 2'][mid_right_index], color = 'brown', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_SE['Component 1'][bottom_left_index], manifold_2D_SE['Component 2'][bottom_left_index], color = 'pink', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_SE['Component 1'][bottom_center_index], manifold_2D_SE['Component 2'][bottom_center_index], color = 'gray', marker='.', linewidth = 5)  
    plt.scatter(manifold_2D_SE['Component 1'][bottom_right_index], manifold_2D_SE['Component 2'][bottom_right_index], color = 'olive', marker='.', linewidth = 5)  
...
def make_cluster(manifold_2D_SE):
    X_clus = []
    for x in range(len(manifold_2D_SE['Component 1'])):
    ...
    for x in set_cluster:
    	for i in range(len(x_labs)):
    	    if x_labs[i] == x:
                set_cluster[x].add(i) 
    counter = 0
    plt.scatter(manifold_2D_SE['Component 1'], manifold_2D_SE['Component 2'], color = colors_set[counter], marker='.', label = save_name.split("/")[0], alpha = 0.2) 
    for x in set_cluster:
        num_in_cluster = 0
        counter += 1 
        for i in set_cluster[x]: 
            if num_in_cluster > 0:
                plt.scatter(manifold_2D_SE['Component 1'][i], manifold_2D_SE['Component 2'][i], color = colors_set[counter], marker='.')  
            else:
                plt.scatter(manifold_2D_SE['Component 1'][i], manifold_2D_SE['Component 2'][i], color = colors_set[counter], marker='.', label = x)  
            num_in_cluster += 1
...
def plot_multi_cluster(manifold_2D_SE_multiple, clustering_DBSCAN_SE, save_name, cluster_names):
    x_labs = clustering_DBSCAN_SE.labels_
    set_cluster = dict()
    for x in x_labs:
        set_cluster[x] = set()
    colors_set = random_colors(len(set_cluster) + len(manifold_2D_SE_multiple))
    for x in set_cluster:
    	for i in range(len(x_labs)): 
    	    if x_labs[i] == x:
                set_cluster[x].add(i) 
    ...
    for manifold_2D_SE in manifold_2D_SE_multiple: 
        for x in range(len(manifold_2D_SE['Component 1'])):
            X_clus.append([manifold_2D_SE['Component 1'][x], manifold_2D_SE['Component 2'][x]])
        plt.scatter(manifold_2D_SE['Component 1'], manifold_2D_SE['Component 2'], color = colors_set[counter], marker='.', label = cluster_names[counter], alpha = 0.2) 
        counter += 1 
    for x in set_cluster:
        num_in_cluster = 0
        for i in set_cluster[x]: 
...
for subdir_name in all_subdirs: 
    if not os.path.isdir(subdir_name) or "Vehicle" not in subdir_name:
        continue
    if not use_SpectralEmbedding:
        if os.path.isfile(subdir_name + "/manifold_2D_SE" + name_extension):
            ...
            SE_matrix = load_object(subdir_name + "/SE_matrix" + name_extension) 
            SpectralEmbedding_list.append(SE_matrix)
            manifold_2D_SE = load_object(subdir_name + "/manifold_2D_SE" + name_extension)  
            
            if draw_SpectralEmbedding:
                plot_SpectralEmbedding(manifold_2D_SE, longs_matrix, lats_matrix, subdir_name + "/plot_SpectralEmbedding" + name_extension + ".png")   
        
            if use_DBSCAN:
                clustering_DBSCAN_SE = make_cluster(manifold_2D_SE)
                save_object(subdir_name + "/clustering_DBSCAN_SE" + name_extension, clustering_DBSCAN_SE)
                
            if not use_DBSCAN and os.path.isfile(subdir_name + "/clustering_DBSCAN_SE" + name_extension):
    ...
    SE_matrix = make_SpectralEmbedding(image_matrix)
    save_object(subdir_name + "/SE_matrix" + name_extension, SE_matrix)
    SpectralEmbedding_list.append(SE_matrix)
    
    manifold_2D_SE = make_manifold(SE_matrix, image_matrix) 
    save_object(subdir_name + "/manifold_2D_SE" + name_extension, manifold_2D_SE) 
...
if use_multi_DBSCAN or draw_multi_DBSCAN:
    for name1 in subdir_names:
        ...
        for name2 in subdir_names:
            multi_manifold_list[name1].append(load_object(name1 + "/multi_SpectralEmbedding" + name_extension + "_" + name1 + "_" + name2)) 
	
if use_multi_DBSCAN:
    for subdir_name in subdir_names:
        clustering_DBSCAN_SE_multi = make_multi_cluster(multi_manifold_list[subdir_name])
        save_object(subdir_name + "/clustering_DBSCAN_SE_multi" + name_extension, clustering_DBSCAN_SE_multi) 
