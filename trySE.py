...
def make_manifold(SE_data_matrix, data_matrix):
    manifold_2Da = SE_data_matrix.fit_transform(data_matrix)
...
def plot_multi_SpectralEmbedding(SE_data_matrix, multi_data_matrix, save_name, title_SpectralEmbedding, label_names):  
    ...
    for data_matrix_index in range(len(multi_data_matrix)):
        ...
        if not os.path.isfile(name_multi_manifold) or use_multi_SpectralEmbedding:
            manifold_2Da = SE_data_matrix.fit_transform(multi_data_matrix[data_matrix_index]) 
            manifold_2D = pd.DataFrame(manifold_2Da, columns = ['Component 1', 'Component 2'])
...
def plot_SpectralEmbedding(manifold_2D, longs_matrix, lats_matrix, save_name):  
...
def plot_cluster_SE(manifold_2D, clustering_DBSCAN_SE, save_name):
...
def plot_multi_cluster(manifold_2D_multiple, clustering_DBSCAN_SE, save_name, cluster_names):
    x_labs = clustering_DBSCAN_SE.labels_
    set_cluster = dict()
    for x in x_labs:
        set_cluster[x] = set()
...
for subdir_name in all_subdirs: 
    ...
    if not use_SpectralEmbedding:
        if os.path.isfile(subdir_name + "/manifold_2D" + name_extension):
            ...
            if draw_SpectralEmbedding:
                plot_SpectralEmbedding(manifold_2D, longs_matrix, lats_matrix, subdir_name + "/plot_SpectralEmbedding" + name_extension + ".png")   
        
            if use_DBSCAN:
                clustering_DBSCAN_SE = make_cluster(manifold_2D)
            ...
            if draw_DBSCAN:
                plot_cluster_SE(manifold_2D, clustering_DBSCAN_SE, subdir_name + "/plot_cluster_SE" + name_extension + ".png")    
    ...
    manifold_2D = make_manifold(SE_matrix, image_matrix) 
