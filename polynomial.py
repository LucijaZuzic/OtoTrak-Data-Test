...
def plot_multi_poly(iso_data_matrix, multi_data_matrix, save_name, title_poly, label_names):  
    ...
    for data_matrix_index in range(len(multi_data_matrix)):
        ...
        if not os.path.isfile(name_multi_manifold) or use_multi_poly:
            ...
            manifold_2D_poly = load_object(name_multi_manifold)
        plt.scatter(manifold_2D_poly['Component 1'], manifold_2D_poly['Component 2'], marker = '.', color = random_color_list[data_matrix_index], label = label_names[data_matrix_index])
...
def plot_poly_poly(manifold_2D_poly, longs_matrix, lats_matrix, save_name):  
    ...
    miny = min(manifold_2D_poly['Component 2'])
    ...
    for x in range(len(manifold_2D_poly['Component 1'])):
        x_dist_right = maxx - manifold_2D_poly['Component 1'][x]
        x_dist_left = manifold_2D_poly['Component 1'][x] - minx
        x_dist_center = abs(manifold_2D_poly['Component 1'][x] - (maxx + minx) / 2)
        y_dist_top = maxy - manifold_2D_poly['Component 2'][x]
        y_dist_mid = abs(manifold_2D_poly['Component 2'][x] - (maxy + miny) / 2)
        y_dist_bottom = manifold_2D_poly['Component 2'][x] - miny
    ...
    print(manifold_2D_poly.head())

    # Show 2D components plot
    plt.figure(figsize = (20, 20))
    plt.subplot(3, 4, 1)
    plt.title(save_name.split("/")[0])
    plt.scatter(manifold_2D_poly['Component 1'], manifold_2D_poly['Component 2'], marker = '.', color = 'cyan')
    plt.scatter(manifold_2D_poly['Component 1'][top_left_index], manifold_2D_poly['Component 2'][top_left_index], color = 'blue', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_poly['Component 1'][top_center_index], manifold_2D_poly['Component 2'][top_center_index], color = 'orange', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_poly['Component 1'][top_right_index], manifold_2D_poly['Component 2'][top_right_index], color = 'green', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_poly['Component 1'][mid_left_index], manifold_2D_poly['Component 2'][mid_left_index], color = 'red', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_poly['Component 1'][mid_center_index], manifold_2D_poly['Component 2'][mid_center_index], color = 'purple', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_poly['Component 1'][mid_right_index], manifold_2D_poly['Component 2'][mid_right_index], color = 'brown', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_poly['Component 1'][bottom_left_index], manifold_2D_poly['Component 2'][bottom_left_index], color = 'pink', marker='.', linewidth = 5) 
    plt.scatter(manifold_2D_poly['Component 1'][bottom_center_index], manifold_2D_poly['Component 2'][bottom_center_index], color = 'gray', marker='.', linewidth = 5)  
    plt.scatter(manifold_2D_poly['Component 1'][bottom_right_index], manifold_2D_poly['Component 2'][bottom_right_index], color = 'olive', marker='.', linewidth = 5)  
...
def make_cluster(manifold_2D_poly):
    X_clus = []
    for x in range(len(manifold_2D_poly['Component 1'])):
    ...
    for x in set_cluster:
    	for i in range(len(x_labs)):
    	    if x_labs[i] == x:
                set_cluster[x].add(i) 
    counter = 0
    plt.scatter(manifold_2D_poly['Component 1'], manifold_2D_poly['Component 2'], color = colors_set[counter], marker='.', label = save_name.split("/")[0], alpha = 0.2) 
    for x in set_cluster:
        num_in_cluster = 0
        counter += 1 
        for i in set_cluster[x]: 
            if num_in_cluster > 0:
                plt.scatter(manifold_2D_poly['Component 1'][i], manifold_2D_poly['Component 2'][i], color = colors_set[counter], marker='.')  
            else:
                plt.scatter(manifold_2D_poly['Component 1'][i], manifold_2D_poly['Component 2'][i], color = colors_set[counter], marker='.', label = x)  
            num_in_cluster += 1
...
def plot_multi_cluster(manifold_2D_poly_multiple, clustering_DBSCAN_poly, save_name, cluster_names):
    x_labs = clustering_DBSCAN_poly.labels_
    set_cluster = dict()
    for x in x_labs:
        set_cluster[x] = set()
    colors_set = random_colors(len(set_cluster) + len(manifold_2D_poly_multiple))
    for x in set_cluster:
    	for i in range(len(x_labs)): 
    	    if x_labs[i] == x:
                set_cluster[x].add(i) 
    ...
    for manifold_2D_poly in manifold_2D_poly_multiple: 
        for x in range(len(manifold_2D_poly['Component 1'])):
            X_clus.append([manifold_2D_poly['Component 1'][x], manifold_2D_poly['Component 2'][x]])
        plt.scatter(manifold_2D_poly['Component 1'], manifold_2D_poly['Component 2'], color = colors_set[counter], marker='.', label = cluster_names[counter], alpha = 0.2) 
        counter += 1 
    for x in set_cluster:
        num_in_cluster = 0
        for i in set_cluster[x]: 
...
def polyresult(xcoord, polycoef):
    ycoord = []
    for xval in xcoord:
        yval = 0
        for number_poly in range(len(polycoef)):
            yval += (xval ** (len(polycoef) - 1 - number_poly)) * polycoef[number_poly]
        ycoord.append(yval)
    return ycoord
...
for subdir_name in all_subdirs: 
    ...
    if not use_poly:
        if os.path.isfile(subdir_name + "/manifold_2D_poly" + name_extension):
            ...
            if use_DBSCAN:
                ...
                save_object(subdir_name + "/clustering_DBSCAN_poly" + name_extension, clustering_DBSCAN_poly)
                
            if not use_DBSCAN and os.path.isfile(subdir_name + "/clustering_DBSCAN_poly" + name_extension):
                clustering_DBSCAN_poly = load_object(subdir_name + "/clustering_DBSCAN_poly" + name_extension)
    ...
    for some_file in all_files:  
        ...
        for x in range(0, len(longitudes) - window_size + 1, step_size):
            ...
            polynomial_result = np.polyfit(longitudes_tmp, latitudes_tmp, window_size - 1)
            #print(polynomial_result)
            #plt.plot(longitudes_tmp, polyresult(longitudes_tmp, polynomial_result))
            #plt.plot(longitudes_tmp, latitudes_tmp)
            #plt.show() 
            poly_matrix.append(polynomial_result)
...
if use_multi_DBSCAN:
    for subdir_name in subdir_names:
        clustering_DBSCAN_poly_multi = make_multi_cluster(multi_manifold_list[subdir_name])
        save_object(subdir_name + "/clustering_DBSCAN_poly_multi" + name_extension, clustering_DBSCAN_poly_multi) 
